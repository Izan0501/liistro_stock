  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in duration-300 p-4 md:p-8 pb-24 md:pb-8">
      <div>
        ...
        <p className="text-sm text-text-secondary mt-1">Manage and track your business capital flow.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Current Capital Card */}
        <div className="glass-card p-6 flex flex-col justify-center relative overflow-hidden">
          <div className="absolute -right-10 -top-10 w-40 h-40 bg-accent-emerald/10 rounded-full blur-3xl pointer-events-none"></div>
          ...
          <div className="text-sm font-medium text-accent-emerald flex items-center gap-1">
            <ArrowUpRight className="w-4 h-4" />
            Live Balance
          </div>
          
          <div className="mt-8 pt-4 border-t border-white/5 flex justify-between text-sm">
            <span className="text-text-secondary">Initial Investment</span>
            <span className="font-medium text-white">{formatCurrency(initialCapital)}</span>
          </div>
        </div>
        ...
        <div className="glass-card p-6">
          ...
          <form onSubmit={handleAdjustCapital} className="space-y-4">
            <div className="flex gap-2 p-1 bg-black/20 rounded-lg border border-white/5">
              <button
                ...
                className={cn(
                  ...
                  operation === 'add' ? "bg-accent-emerald text-white shadow-sm" : "text-text-secondary hover:text-white"
                )}
              >
              ...
              <button
                ...
                className={cn(
                  "flex-1 flex items-center justify-center gap-2 py-2 text-sm font-medium rounded-md transition-all",
                  operation === 'subtract' ? "bg-accent-red text-white shadow-sm" : "text-text-secondary hover:text-white"
                )}
              >
            ...
            <div>
              <label className="text-sm font-medium text-text-secondary block mb-1.5">Amount (USD)</label>
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full bg-black/20 border border-white/10 rounded-lg pl-9 pr-4 py-2.5 focus:outline-none focus:border-accent-indigo text-white" 
                  placeholder="0.00"
                />
              </div>
            </div>
            ...
            <div>
              <label className="text-sm font-medium text-text-secondary block mb-1.5">Reason / Concept</label>
              <input 
                type="text" 
                required
                value={reason}
            ...
            <button 
              ...
              className={cn(
                "w-full flex items-center justify-center gap-2 py-3 rounded-lg font-bold text-white transition-all shadow-lg disabled:opacity-50 mt-2",
                operation === 'add' ? "bg-accent-emerald hover:brightness-110 shadow-[0_0_15px_rgba(16,185,129,0.3)]" : "bg-accent-red hover:brightness-110 shadow-[0_0_15px_rgba(239,68,68,0.3)]"
              )}
            >
