...
const TriggerButton = React.forwardRef<HTMLButtonElement, TriggerButtonProps>(({ date, className, ...props }, ref) => (
...
const PresetsMenu = ({ presets }: PresetsMenuProps) => (
...
const CalendarUI = ({ months, date, setDate }: CalendarUIProps) => (
  <>
    <style>{`
      ...
      <Popover.Root open={open} onOpenChange={setOpen}>
        ...
        <Popover.Portal>
          ...
          >
            ...
            <CalendarUI months={2} date={date} setDate={setDate} />
          </Popover.Content>
        </Popover.Portal>
      </Popover.Root>
    )
        ...
        <Drawer.Content className="fixed bottom-0 left-0 right-0 z-[10000] mt-24 flex flex-col rounded-t-[10px] bg-white dark:bg-slate-950 border-t border-slate-200 dark:border-slate-800 text-slate-950 dark:text-white max-h-[96vh]">
          <div className="p-4 rounded-t-[10px] flex-1 overflow-y-auto w-full flex flex-col gap-6 items-center">
            ...
            <div className="w-full flex justify-center pb-8">
              <CalendarUI months={1} date={date} setDate={setDate} />
            </div>
          </div>
        </Drawer.Content>
      </Drawer.Portal>
