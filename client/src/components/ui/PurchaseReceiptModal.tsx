    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-[0_20px_50px_rgb(0,0,0,0.1)] dark:shadow-2xl w-full max-w-lg transition-colors overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh] z-50 opacity-100">
        ...
        <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50">
          <h3 className="text-lg font-bold text-slate-950 dark:text-white tracking-tight">Purchase Detail / Compra</h3>
          <button 
            onClick={onClose}
          ...
          <div className="p-6 bg-white dark:bg-slate-900/80 font-mono text-sm print:bg-white print:text-black text-slate-950 dark:text-slate-300">
            ...
            <div className="space-y-2 mb-6 text-slate-500 dark:text-slate-400 print:text-gray-800">
              <div className="flex justify-between">
                <span>Nº COMPRA:</span>
                <span className="text-slate-950 dark:text-white print:text-black font-medium">{purchase.id?.split('-')[0].toUpperCase()}</span>
              </div>
              <div className="flex justify-between">
                <span>FECHA:</span>
                <span className="text-slate-950 dark:text-white print:text-black font-medium">{formatDate(purchaseDate)}</span>
              </div>
              <div className="flex justify-between">
                <span>PROVEEDOR:</span>
                <span className="text-slate-950 dark:text-white print:text-black font-medium">{supplierName}</span>
              </div>
            </div>

            {(purchase.reason || purchase.notes) && (
              <div className="mb-6 p-3 rounded-md bg-slate-100 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 text-sm print:bg-gray-100 print:border-gray-300">
                <span className="font-bold text-slate-700 dark:text-slate-300 print:text-gray-800 block mb-1">Razón / Notas:</span>
                <p className="text-slate-600 dark:text-slate-400 italic print:text-gray-600">
                  {purchase.reason || purchase.notes}
                </p>
              </div>
            )}
            ...
            <div className="flex justify-between items-center text-lg font-black text-slate-950 dark:text-white print:text-black">
              ...
              <span className="text-red-600 dark:text-red-400 print:text-red-600">{formatCurrency(Number(purchase.total_amount || purchase.total_cost || 0))}</span>
            </div>
          </div>
        </div>
